...
export default function PopularRoutes() {
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 w-full h-full">
      <h2 className="text-sm font-bold text-gray-900 mb-8">Popular Routes</h2>
      
      <div className="space-y-8">
        {routes.map((route) => (
          <div key={route.name}>
            <div className="flex justify-between items-center mb-3 text-xs font-bold">
              <span className="text-gray-900 text-xs">{route.name}</span>
              <span className="text-gray-500 text-[10px]">{route.percentage}%</span>
            </div>
